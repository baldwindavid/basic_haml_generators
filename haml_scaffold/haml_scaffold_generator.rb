class ScaffoldingSandbox
  ...
  #HAML Edit
  # Updated this to use haml syntax
  def default_input_block
    Proc.new { |record, column| "%p\n  %label{:for => \"#{record}_#{column.name}\"} #{column.human_name}\n  %br\n  = #{input(record, column.name)}\n" }
  end

end

# Updated these to use haml syntax
...
class HamlScaffoldGenerator < Rails::Generator::NamedBase
  ...
  def manifest
    record do |m|
      ...
      # Scaffolded forms.
      m.complex_template "form.haml",
        File.join('app/views',
                  controller_class_path,
                  controller_file_name,
                  "_form.haml"),
        :insert => 'form_scaffolding.haml',
      ...
      m.template 'layout.haml',
                  File.join('app/views/layouts',
                            controller_class_path,
                            "#{controller_file_name}.haml")
      ...
      unscaffolded_actions.each do |action|
        path = File.join('app/views',
                          ...
                          controller_file_name,
                          "#{action}.haml")
        m.template "controller:view.haml", path,
  ...
  protected
    ...
    def banner
      "Usage: #{$0} haml_scaffold ModelName [ControllerName] [action, ...]"
    end
